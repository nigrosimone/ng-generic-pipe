...
export class NgGenericDirective<T> {
  ...
  set ngGenericPipeMethod(value: T) {
    ...
    if (this.lastFn !== value) {
      this.lastFn = value;
      this.context.$implicit = this.context.ngGenericPipe = value.bind(
        (this.cdRef as EmbeddedViewRef<Type<unknown>>).context,
      );
    }
  }
  ...
  constructor(
    ...
    templateRef: TemplateRef<NgGenericContext<T>>,
  ) {
  ...
  static ngTemplateContextGuard<T>(
    _dir: NgGenericDirective<T>,
    _ctx: unknown,
  ): _ctx is NgGenericContext<T> {
    return true;
  }
}
